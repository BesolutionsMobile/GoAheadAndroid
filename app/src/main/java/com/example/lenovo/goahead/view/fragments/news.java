...
public class news extends Fragment implements interfaceMVPP.interfaces.View {
    ...
    void getAllData(String id)
    {
        ...
        JsonObjectRequest jsonObjectRequest=new JsonObjectRequest(Request.Method.GET, "http://coderg.org/goahead_en/Category/view/82984218/951735/"+id+"/"+savedId.getId(getActivity()), null, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
                try {
                    if(response.getString("status").equals("1")) {
